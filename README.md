...
The homepage activity counters use the database directly. `PUBLIC_STATS_START_DATE` keeps imported demonstration rows out of the public totals, and `PUBLIC_STATS_SEEDED_CATEGORY_LIMIT` keeps the built-in 20-category catalog out of the user-created category total. New approved artisans, categories, and reviews created after that date are counted automatically. The regions counter is fixed at Ghana&apos;s 16 administrative regions.
