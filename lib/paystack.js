...
function getPaystackSecretKey() {
  ...
  if (!key || key.trim() === '' || key === 'MY_PAYSTACK_SECRET_KEY' || key.startsWith('your_')) {
    return null;
  }
  return key.trim();
}
...
function isDemoModeEnabled() {
  return process.env.PAYSTACK_DEMO_MODE === 'true' || process.env.PAYSTACK_DEMO_MODE === '1';
}

function ensureLivePaymentConfiguration() {
  ...
  if (process.env.NODE_ENV === 'production') {
    throw new Error('Live Paystack payments are not configured. Add PAYSTACK_SECRET_KEY in the production environment before enabling checkout.');
  }

  if (!isDemoModeEnabled()) {
    throw new Error('Paystack is not configured. Add PAYSTACK_SECRET_KEY or explicitly enable PAYSTACK_DEMO_MODE for local sandbox testing.');
  }
...
export async function initializeTransaction({ email, amount_ghs, reference, callback_url }) {
  ...
  const usingLiveGateway = ensureLivePaymentConfiguration();
  ...
  if (process.env.NODE_ENV === 'production') {
    throw new Error('Live Paystack is not configured. Add PAYSTACK_SECRET_KEY to the hosting environment.');
  }

  if (!isDemoModeEnabled()) {
    throw new Error('Paystack is not configured. Add PAYSTACK_SECRET_KEY or explicitly enable PAYSTACK_DEMO_MODE for local demonstrations.');
  }

  // Local-only fallback is opt-in and must never be mistaken for a real payment.
 ...
 * @param {number} [expectedAmountGhs] - Expected amount for the explicit local demo mode
 * @returns {Promise<Object>} - Decoded transaction data from Paystack
 */
export async function verifyTransaction(reference, expectedAmountGhs) {
  const secretKey = getPaystackSecretKey();
  ...
  if (process.env.NODE_ENV === 'production' || !isDemoModeEnabled()) {
    throw new Error('Live Paystack verification is not configured. Add PAYSTACK_SECRET_KEY to the hosting environment.');
  }

  // Local-only fallback verification for development screenshots and demonstrations.
  console.log('PAYSTACK_SECRET_KEY not set. Verifying sandbox transaction reference:', reference);
  return {
    status: 'success',
    reference,
    amount: Math.round(Number(expectedAmountGhs || 0) * 100),
    currency: 'GHS',
