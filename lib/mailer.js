...
function getMailerConfigInternal() {
  ...
  const isConfigured = Boolean(host && port && user && pass && !looksLikePlaceholder(user) && !looksLikePlaceholder(pass));
...
function getTransporter() {
  if (!transporter) {
    const { host, port, user, pass, isConfigured } = getMailerConfigInternal();
    const isProduction = process.env.NODE_ENV === 'production';

    if (!isConfigured) {
      const message =
        'Mail server environment variables are not fully configured or contain placeholder values. Email functionality cannot run in this environment.';
      if (isProduction) {
        console.error(message);
        throw new Error('SMTP mailer is not configured for production. Set EMAIL_HOST, EMAIL_PORT, EMAIL_USER, and EMAIL_PASS.');
      }

      console.warn(`${message} Email will be logged to the server console in non-production mode.`);
      return null;
    }
...
export function getMailerSettings() {
  return getMailerConfigInternal();
}

export function isMockEmailMode() {
  return !getMailerConfigInternal().isConfigured;
}

export async function sendEmail({ to, subject, html }) {
  const activeTransporter = getTransporter();
  const { from } = getMailerSettings();

  if (!activeTransporter) {
    if (process.env.NODE_ENV === 'production') {
      throw new Error('SMTP mailer unavailable in production. Please configure EMAIL_HOST, EMAIL_PORT, EMAIL_USER, and EMAIL_PASS.');
    }
