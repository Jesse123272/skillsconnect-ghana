...
async function ensureMissingColumns(pool, tableName, columns) {
  try {
    const [existingColumns] = await pool.query('SHOW COLUMNS FROM ??', [tableName]);
    const existingNames = new Set(existingColumns.map((column) => column.Field));

    for (const column of columns) {
      if (!existingNames.has(column.name)) {
        await pool.query(`ALTER TABLE ?? ADD COLUMN ?? ${column.definition}`, [tableName, column.name, column.definition]);
      }
    }
  } catch (error) {
