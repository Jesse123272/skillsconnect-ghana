...
// Immediately prefer SQLite fallback if DB_HOST is not explicitly set in environment
if (!process.env.DB_HOST || process.env.DB_HOST === '127.0.0.1' || process.env.USE_SQLITE === 'true') {
...
async function getSqlJsDb() {
  ...
  initPromise = (async () => {
    ...
    try {
      ...
      loadSqlJs = sqlJsModule.default || sqlJsModule;
    } catch (err) {
      console.error('Failed to dynamically import sql.js:', err);
      throw err;
    }

    const SQL = await loadSqlJs({
      locateFile: file => {
        if (file.endsWith('sql-wasm.wasm')) {
          return resolveProjectPath('public', 'sql-wasm.wasm');
        }
        return resolveProjectPath('node_modules', 'sql.js', 'dist', file);
      }
    });
