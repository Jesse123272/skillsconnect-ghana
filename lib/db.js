import { querySqliteFallback } from './sqlite-fallback.mjs';
...
async function ensureRequiredTables(pool) {
  try {
    ...
    if (!tables.has('guarantors')) {
      await pool.query(`CREATE TABLE IF NOT EXISTS guarantors (
    ...
    if (!tables.has('job_milestones')) {
      await pool.query(`CREATE TABLE IF NOT EXISTS job_milestones (
        milestone_id INT AUTO_INCREMENT PRIMARY KEY,
        enquiry_id INT NOT NULL,
        title VARCHAR(180) NOT NULL,
        description TEXT DEFAULT NULL,
        amount DECIMAL(10,2) DEFAULT 0.00,
        status ENUM('pending', 'in_progress', 'completed', 'released', 'cancelled') DEFAULT 'pending',
        created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
        updated_at DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    ...
    await ensureMissingColumns(pool, 'users', [
      ...
      { name: 'preferred_language', definition: 'VARCHAR(16) DEFAULT "en"' },
      { name: 'locale', definition: 'VARCHAR(16) DEFAULT "en"' }
    ]);
    await ensureMissingColumns(pool, 'artisan_profiles', [
      { name: 'starting_price', definition: 'DECIMAL(10,2) DEFAULT NULL' },
      { name: 'is_available', definition: 'TINYINT DEFAULT 1' },
      { name: 'accepts_emergency', definition: 'TINYINT DEFAULT 0' },
      { name: 'ghana_card_verified', definition: 'TINYINT DEFAULT 0' },
      { name: 'police_checked', definition: 'TINYINT DEFAULT 0' },
      { name: 'trade_certified', definition: 'TINYINT DEFAULT 0' }
    ]);
    await pool.query("ALTER TABLE enquiries MODIFY COLUMN status ENUM('pending', 'replied', 'in_progress', 'completed', 'closed', 'cancelled') DEFAULT 'pending'");
    await ensureMissingColumns(pool, 'enquiries', [
      { name: 'completed_at', definition: 'DATETIME DEFAULT NULL' },
      { name: 'completed_by', definition: 'INT DEFAULT NULL' }
    ]);
  } catch (error) {
...
export async function query(sql, params = []) {
  ...
  if (useSqliteFallback) {
    if (isProduction) {
      throw new Error('Production database is unavailable. Configure a reachable MySQL database before serving authentication requests.');
    }
  ...
  try {
    ...
    try {
      ...
      if (!['PROTOCOL_CONNECTION_LOST', 'ECONNRESET', 'POOL_CLOSED'].includes(error?.code) && error?.message !== 'Pool is closed.') {
        throw error;
      }

      // Railway can close an idle connection between serverless invocations.
      // Recreate the pool once so the request can recover without falling back.
      await pool.end().catch(() => {});
      mysqlPool = null;
      mysqlSchemaReady = false;
      const refreshedPool = await getMysqlPool();
      if (!refreshedPool) throw error;
      const [rows] = await refreshedPool.execute(sql, params);
      return rows;
    }
  } catch (error) {
