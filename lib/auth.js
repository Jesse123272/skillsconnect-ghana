...
export async function signGoogleVerificationChallenge(payload) {
...
export async function verifyGoogleVerificationChallenge(token) {
...
export function setAuthCookie(response, token, options = {}) {
  ...
  const maxAge = typeof options.maxAge === 'number' ? options.maxAge : 7 * 24 * 60 * 60;
 ...
 */
export function clearAuthCookie(response, options = {}) {
  if (!response || !response.cookies) {
