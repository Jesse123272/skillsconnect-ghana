...
function getDefaultSchemaStatements() {
  return [
    ...
    `CREATE TABLE IF NOT EXISTS artisan_profiles (
      ...
      accepts_emergency INTEGER DEFAULT 0,
      ghana_card_verified INTEGER DEFAULT 0,
      police_checked INTEGER DEFAULT 0,
      trade_certified INTEGER DEFAULT 0,
    ...
    try {
      try {
        ...
        if (!userColNames.includes('google_id')) {
          console.log('SQLite migration: adding users.google_id column');
          db.run("ALTER TABLE users ADD COLUMN google_id TEXT DEFAULT NULL");
        }
      ...
      try {
        const artisanCols = db.exec("PRAGMA table_info('artisan_profiles')");
        const artisanColNames = (artisanCols && artisanCols[0] && artisanCols[0].values) ? artisanCols[0].values.map(r => r[1]) : [];
        if (!artisanColNames.includes('starting_price')) {
          console.log('SQLite migration: adding artisan_profiles.starting_price column');
          db.run("ALTER TABLE artisan_profiles ADD COLUMN starting_price REAL DEFAULT NULL");
        }
        if (!artisanColNames.includes('is_available')) {
          console.log('SQLite migration: adding artisan_profiles.is_available column');
          db.run("ALTER TABLE artisan_profiles ADD COLUMN is_available INTEGER DEFAULT 1");
        }
        if (!artisanColNames.includes('accepts_emergency')) {
          console.log('SQLite migration: adding artisan_profiles.accepts_emergency column');
          db.run("ALTER TABLE artisan_profiles ADD COLUMN accepts_emergency INTEGER DEFAULT 0");
        }
        if (!artisanColNames.includes('ghana_card_verified')) {
          console.log('SQLite migration: adding artisan_profiles.ghana_card_verified column');
          db.run("ALTER TABLE artisan_profiles ADD COLUMN ghana_card_verified INTEGER DEFAULT 0");
        }
        if (!artisanColNames.includes('police_checked')) {
          console.log('SQLite migration: adding artisan_profiles.police_checked column');
          db.run("ALTER TABLE artisan_profiles ADD COLUMN police_checked INTEGER DEFAULT 0");
        }
        if (!artisanColNames.includes('trade_certified')) {
          console.log('SQLite migration: adding artisan_profiles.trade_certified column');
          db.run("ALTER TABLE artisan_profiles ADD COLUMN trade_certified INTEGER DEFAULT 0");
        }
      } catch (e) {
        console.warn('SQLite migration (artisan_profiles) check failed:', e?.message || e);
      }

      try {
        const enquiryCols = db.exec("PRAGMA table_info('enquiries')");
        const enquiryColNames = (enquiryCols && enquiryCols[0] && enquiryCols[0].values) ? enquiryCols[0].values.map(r => r[1]) : [];
        if (!enquiryColNames.includes('completed_at')) db.run("ALTER TABLE enquiries ADD COLUMN completed_at TEXT DEFAULT NULL");
        if (!enquiryColNames.includes('completed_by')) db.run("ALTER TABLE enquiries ADD COLUMN completed_by INTEGER DEFAULT NULL");
      } catch (e) {
        console.warn('SQLite migration (enquiries) check failed:', e?.message || e);
      }
      ...
      try {
        ...
        console.warn('SQLite migration (guarantors) check failed:', e?.message || e);
      }
    } catch (err) {
